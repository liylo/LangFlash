...
#             gaussian_lang_weight = repeat(gaussians.lang_weights, "b g l -> (b v) g l", v=v) if hasattr(gaussians, 'lang_weights') else None,
#         )
#         color = rearrange(color, "(b v) c h w -> b v c h w", b=b, v=v)
#         # depth = rearrange(depth, "(b v) h w -> b v h w", b=b, v=v)
#         if lang is not None:
#             lang = rearrange(lang, "(b v) l h w -> b v l h w", b=b, v=v)
#         return DecoderOutput(color, depth, lang)
...
class DecoderSplattingCUDA(Decoder[DecoderSplattingCUDACfg]):
    ...
    def forward(
        ...
        depth = rearrange(depth, "(b v) h w -> b v h w", b=b, v=v)
        return DecoderOutput(color, depth,None)