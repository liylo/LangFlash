...
class AsymmetricCroCoLang(CroCoNet):
    ...
    def __init__(self, cfg: BackboneCrocoCfg, d_in: int) -> None:
        ...
        # functools.partial(<class 'torch.nn.modules.normalization.LayerNorm'>, eps=1e-06)
        dec_embed_dim, dec_num_heads, dec_depth, mlp_ratio, norm_im2_in_dec = (768,  12,  3, 4,  True)
            
        self.lang_cond = nn.ModuleList([
            DecoderBlock(dec_embed_dim, dec_num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=nn.LayerNorm, norm_mem=norm_im2_in_dec, rope=None)
            for i in range(dec_depth)])
        self.lang_cond2 = nn.ModuleList([
            DecoderBlock(dec_embed_dim, dec_num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=nn.LayerNorm, norm_mem=norm_im2_in_dec, rope=None)
            for i in range(dec_depth)])
        self.mapper = nn.Linear(512,768)
        self.squeeze = nn.Linear(256 + 16, 256)
    ...
    def _decoder(self, f1, pos1, f2, pos2, extra_embed1=None, extra_embed2=None, lang_1 = None, lang_2=None, codebook=None, mask =  None):
        if mask is not None:
            mask1 = mask[:, 0]
            mask2 = mask[:, 1]
        else:
        ...
        # if lang_1 is not None and lang_2 is not None:
        #     rep = rearrange(codebook,'b v d n -> b v n d')
        #     lang_1 = self.mapper(lang_1)
        #     lang_1 = torch.cat((lang_1,f1[:,-1:]),dim=1)
        #     rep_new = self.mapper(rep[:,0,:,:])
        #     lang_1 = torch.cat((lang_1,rep_new),dim=1)
        #     lang_2 = self.mapper(lang_2)
        #     lang_2 = torch.cat((lang_2,f2[:,-1:]),dim=1)
        #     _, n_k, _ = lang_2.shape
        #     mask_1 = mask1+n_k
        #     mask_2 = mask2+n_k
        #     rep_new = self.mapper(rep[:,1,:,:])
        #     lang_2 = torch.cat((lang_2,rep_new),dim=1)
        ...
        for index, (blk1, blk2) in enumerate(zip(self.dec_blocks, self.dec_blocks2)):
            # img1 side
            f1, _ = blk1(*final_output[-1][::+1], pos1, pos2)
            # img2 side
            f2, _ = blk2(*final_output[-1][::-1], pos2, pos1)
            # TODO: replace with actual language embeddings
            # if index in [3,7,11] and lang_1 is not None and lang_2 is not None:
            if lang_1 is not None and lang_2 is not None:
                if index in [3,7,11]:
                    f1, _ = self.lang_cond[(index//4)](f1,lang_1,pos1,pos1,mask_1)
                    f2, _ = self.lang_cond2[(index//4)](f1,lang_2,pos2,pos2,mask_2)
            # store the result
            final_output.append((f1, f2))
    ...
    def forward(self,
        ...
        # TODO: replace with actual language embeddings
        # lang_1 = torch.rand((b,256,512), device=device)
        # lang_2 = torch.rand((b,256,512), device=device)
        mask = context.get('mask', None)
        if context.get('lang', None) is not None:
        
            lang = context['lang']
            B, C, D, H, W = lang.shape  # [1, 2, 512, 256, 256]
            lang_reshaped = lang.view(B * C * D, 1, H, W)  # [1024, 1, 256, 256]
            lang_downsampled = nn.functional.interpolate(lang_reshaped, size=(16, 16), mode='bilinear', align_corners=False)
            lang_result = lang_downsampled.view(B, C, D, 16, 16)  # [1, 2, 512, 16, 16]
            lang_result = rearrange(lang_result, 'b c d h w -> b c (h w) d')  # [1, 2, 256, 512]
            lang_1 = lang_result[:, 0, :, :].to(torch.float32)  # [1, 256, 512]
            lang_2 = lang_result[:, 1, :, :].to(torch.float32)  # [1, 256, 512]
            
        else:
            lang_1 = None
            lang_2 = None
            
        codebook = context.get('codebook', None)
        ...
        if self.intrinsics_embed_loc == 'decoder':
            ...
            dec1, dec2 = self._decoder(feat1, pos1, feat2, pos2,lang_1 = lang_1,lang_2 = lang_2,codebook=codebook,mask=mask)
